...
void UART_Init(int idx, int baud)
{
  ...
  IO_Init(_uCfg[idx].ioTx);
  
  // RX ucu yapýlandýrmasý
  IO_Init(_uCfg[idx].ioRx);
...
void UART_Send(int idx, unsigned char val)
{
  ...
  while(!USART_GetFlagStatus(_uCfg[idx].pUSART, USART_FLAG_TXE)); // TXE 1 olduðunda TDR boþaldý demektir.Yeni data yazýlabilir.
...
{ // RXNE : veri alma registerý boþ deðil, yani veri var.veri hazýr.1 olduðunda veri almaya hazýr.
